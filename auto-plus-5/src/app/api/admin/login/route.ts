...
export async function POST(request: NextRequest) {
  ...
  const { data } = await supabase.from('admin_access_reveals').select('code_hash,expires_at').eq('code_hash', codeHash).is('consumed_at', null).order('created_at', { ascending: false }).limit(1).maybeSingle()
  ...
  const { error } = await supabase.from('admin_sessions').insert({ session_hash: sessionHash, expires_at: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toISOString() })
